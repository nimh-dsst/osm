...
from osm.converters.xml_json import xml_to_json
...
def convert_pdf_to_xml(file_path, output_file_path):
    ...
    try:
        ...
        Path(f"docs/examples/sciencebeam_xml_outputs/{output_file_path}").write_text(
            xml_content
        )
        ...
        xml_to_json(
            hash_file(file_path),
            f"docs/examples/sciencebeam_xml_outputs/{output_file_path}",
        )
