...
def setup_and_teardown():
    ...
    xml_folder = "docs/examples/sciencebeam_xml_outputs"
    file = "Dafflon et al. 2024 - Reliability and predictability of phenotype information from functional connectivity in large imaging datasets.pdf"
    output_file = "test_output_file.xml"

    yield pdfs_folder, xml_folder, file, output_file

    # Teardown: Remove the generated XML file
    output_file_path = Path(f"{xml_folder}/{output_file}")
    if output_file_path.exists():
        output_file_path.unlink()


def test_pdf_xml_json_command(setup_and_teardown, mock_socket, mock_requests_convert):
    pdfs_folder, xml_folder, file, output_file = setup_and_teardown
    ...
    assert Path(f"{xml_folder}/{output_file}").exists()
