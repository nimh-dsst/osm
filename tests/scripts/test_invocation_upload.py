...
from scripts.invocation_upload import read_data, upload_data, transform_data, main
import logging
...
def mock_read_data(mocker):
    ...
    mocker.patch("scripts.invocation_upload.read_data",
...
def mock_upload_data_success(monkeypatch, mocker):
    ...
    monkeypatch.setattr("scripts.invocation_upload.upload_data",
...
async def test_upload_data_success(mock_database):
    ...
    await upload_data(invocation_list, mongo_uri, db_name)
...
async def test_upload_data_failure(mock_database, caplog):
    ...
    with pytest.raises(TypeError,  match="NoneType' object is not iterable"):
        await upload_data(invocation_list, None, None)


def test_main(mock_read_data, mock_upload_data_success, capfd):
    main(data_path="all_indicators.feather")
...
def test_main_failure(mock_read_data):
    with pytest.raises(TypeError, match="NoneType' object is not iterable"):
        main(data_path=None)
